...
export default function ChatScreen() {
  ...
  const load = async () => {
    ...
    try { const { data } = await api.get(`/chat/${empresaId}`); setMsgs(data); }
    catch {}
    finally { setLoading(false); }
  };

  useEffect(() => { load(); const t = setInterval(load, 5000); return () => clearInterval(t); }, []);
  useEffect(() => { if (msgs.length) flatRef.current?.scrollToEnd({ animated: true }); }, [msgs]);
  ...
  if (!empresaId) return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.center}><Text style={s.empty}>Você não está associado a uma empresa</Text></View>
    </SafeAreaView>
  );
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
        {loading
          ...
          : (
            <FlatList
              ...
              ListEmptyComponent={<Text style={[s.empty, { marginTop: 40 }]}>Nenhuma mensagem ainda</Text>}
              renderItem={({ item }) => {
                const mine = item.from_role === user?.role;
                return (
                  <View style={[s.bubble, mine ? s.bubbleMine : s.bubbleOther]}>
                    {!mine && <Text style={s.bubbleRole}>Recepção</Text>}
                    <Text style={[s.bubbleText, mine ? { color: '#fff' } : { color: C.text }]}>{item.texto}</Text>
                    <Text style={s.bubbleTime}>{new Date(item.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</Text>
                  </View>
                );
              }}
            />
        ...
        <View style={s.inputBar}>
          <TextInput
            style={s.input} placeholder="Digite uma mensagem..." placeholderTextColor={C.muted}
            value={text} onChangeText={setText} multiline
            onSubmitEditing={send} returnKeyType="send"
          />
          <TouchableOpacity style={[s.sendBtn, !text.trim() && { opacity: 0.4 }]} onPress={send} disabled={!text.trim()}>
...
const s = StyleSheet.create({
  ...
  bubbleTime: { color: 'rgba(255,255,255,0.5)', fontSize: 10, alignSelf: 'flex-end' },
